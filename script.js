...
function renderResult(data) {
  ...
  const cls =
    verdict === "In Scope"
      ? "in"
      : verdict === "Partially Out of Scope"
        ? "partial"
        : "out";
  ...
  // Banner amount: show 'Clear!' only when verdict is In Scope and no risk.
  ...
  // Banner subtext should reflect both verdict and risk
  if (verdict === "In Scope") {
    document.getElementById("riskSub").textContent =
      risk > 0 ? "Revenue at risk — unpaid work" : "Request is within scope";
  } else if (verdict === "Partially Out of Scope") {
    document.getElementById("riskSub").textContent =
      risk > 0
        ? "Revenue at risk — unpaid work"
        : "Partially out of scope — no extra hours estimated";
  } else {
    document.getElementById("riskSub").textContent =
      risk > 0
        ? "Revenue at risk — unpaid work"
        : "Out of scope — no extra hours estimated";
  }
